...
def skeleton_recognition(cap, time_11, time_12, time_31, time_32, time_4):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:  # установка порогов доверия
        #while cap.isOpened():
        local_ret, local_frame = cap.read()
        ...
        try:
            ...
            cv2.circle(image,
                       (int(avg_point_shoulder[0] * local_frame.shape[1]), int(avg_point_shoulder[1] * local_frame.shape[0])),
                       5, (0, 0, 255), -1)
            # отрисовка средней точки тазового пояса
            avg_point_hip = ((left_hip[0] + right_hip[0]) / 2, (left_hip[1] + right_hip[1]) / 2)
            cv2.circle(image,
                       (int(avg_point_hip[0] * local_frame.shape[1]), int(avg_point_hip[1] * local_frame.shape[0])), 5,
                       (0, 0, 255), -1)

            cv2.line(image,
                     (int(avg_point_shoulder[0] * local_frame.shape[1]), int(avg_point_shoulder[1] * local_frame.shape[0])),
                     (int(avg_point_hip[0] * local_frame.shape[1]), int(avg_point_hip[1] * local_frame.shape[0])), (255, 0, 0),
                     2)
            ...
            while flag_spine != True:
                print(code_0)
                time.sleep(5)
                min_spine_length = math.sqrt((avg_point_shoulder[0] - avg_point_hip[0]) ** 2 + (avg_point_shoulder[1] - avg_point_hip[1]) ** 2)
                print(code_1)
                flag_spine = True
            """
        ...
        return image, time_11, time_12, time_31, time_32, time_4
